...
const ScrollMarker = ({
  ...
  const markerClass = current
    ? "inline-block h-4 w-4 rounded-full border-2 border-neutral-700 bg-white"
  ...
  return (
    <div className="my-8 flex flex-col gap-16">
      ...
      <div className="relative mx-auto w-full max-w-2xl rounded-lg border bg-neutral-900 p-6 sm:p-6">
          ...
          <div className="relative my-2 flex items-center justify-center gap-4 sm:gap-8">
            {[0, 1, 2].map((i) => (
              <ScrollMarker
                ...
                current={i === 1}
                label={
                  i === 1 ? "::scroll-marker:target-current" : "::scroll-marker"
                }
              />
            ))}
          </div>
        ...
        {/* Labels */}
        <span className="absolute left-2 top-2 text-xs text-white/80">
